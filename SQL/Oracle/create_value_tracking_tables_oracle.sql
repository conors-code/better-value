...
create table KEY_VALUE_INDICATOR_TARGET_VALUES (
    ...
    THRESHOLD_VALUE_FOR_RAG VARCHAR2(255) not null,
    constraint FK_TARGET_ foreign key (INDICATOR_ID) references KEY_VALUE_INDICATORS (INDICATOR_ID)
);
...
create table KEY_VALUE_INDICATOR_ACTUALS (
    ...
    ACTUAL_VALUE VARCHAR2(255) not null,
    IS_PROMISED_VALUE NUMBER(1) default 0 not null, -- 0 = false, 1 = true. Default to measured unless in future
    DATE_VALUE_EFFECTIVE DATE not null,
    ACTUAL_VALUE_SOURCE VARCHAR2(255) default 'promised' not null,
